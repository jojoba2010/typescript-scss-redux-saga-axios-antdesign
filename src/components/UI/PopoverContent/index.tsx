...
const PopoverContent = ({ setVisible, children }) => {
    ...
    useEffect(() => {
        ...
        function handleClickOutsideUserMenu(event) {
            if (
                ...
                // Next condition is for checking click on clear select icon
                // we can just check the path value for clear icon
                !event?.target?.outerHTML?.includes(
